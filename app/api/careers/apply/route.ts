...
const WP_APPLY_URL = "https://blog.clanap.com/wp-json/careers/v1/apply";

export async function POST(request: NextRequest) {
  try {
    const incomingForm = await request.formData();

    const wpResponse = await fetch(WP_APPLY_URL, {
      method: "POST",
      body: incomingForm,
    });
    ...
    const name = String(incomingForm.get("name") ?? "");
    const email = String(incomingForm.get("email") ?? "");
    const phone = String(incomingForm.get("phone") ?? "");
    const linkedin = String(incomingForm.get("linkedin") ?? "");
    const coverLetter = String(incomingForm.get("coverLetter") ?? "");
    const jobTitle = String(incomingForm.get("jobTitle") ?? "General Application");
...
async function sendApplicationNotifications({
  name,
  email,
  phone,
  linkedin,
    ...
    html: `
      <div style="max-width: 600px; margin: 0 auto; padding: 20px; font-family: Arial, sans-serif; line-height: 1.6;">
        ...
        <div style="background-color: #fff8ec; border-left: 4px solid #ed8c01; padding: 15px; margin-bottom: 20px;">
          <h2 style="color: #4a2801; margin: 0 0 15px 0;">New Job Application</h2>
        ...
        <div style="font-size: 12px; color: #6B7280; text-align: center; margin-top: 30px;">
          <p>This application was submitted through the Clan-AP Technologies careers page.</p>
        </div>
      </div>
    `,
  });
  ...
  await transporter.sendMail({
    ...
    to: email,
    subject: `We've received your application${jobTitle ? ` for ${jobTitle}` : ""}`,
    html: `
      <div style="max-width: 600px; margin: 0 auto; padding: 20px; font-family: Arial, sans-serif; line-height: 1.6;">
        ...
        <div style="background-color: white; border-radius: 8px; box-shadow: 0 4px 15px rgba(237,140,1,0.12); padding: 30px; margin-bottom: 16px; border-top: 5px solid #ed8c01;">
          <h2 style="color: #4a2801; margin: 0 0 20px 0; text-align: center;">We've received your application!</h2>

          <p style="color: #374151;">Hi ${name},</p>
          <p style="color: #374151;">Thanks for applying${
            jobTitle ? ` to <strong>${jobTitle}</strong>` : ""
          } at Clan-AP Technologies Private Limited. Our team has received your application and will review it shortly. We'll reach out if there's a fit for the role.</p>
